...
def get_sharepoint_item_attachments(item_id):
    """
    Fetch attachments for a specific SharePoint list item.
    """
    token = get_access_token()
    site_url = "https://hamdaz1.sharepoint.com/sites/Test"
    list_name = "Quotes"

    # First get the site ID using path
    site_resp = requests.get(
        f"{GRAPH_API_ENDPOINT}/sites/{site_url}",
        headers={"Authorization": f"Bearer {token}"}
    )
    site_resp.raise_for_status()
    site_id = site_resp.json()["id"]

    # Then get the list ID
    list_resp = requests.get(
        f"{GRAPH_API_ENDPOINT}/sites/{site_id}/lists/{list_name}",
        headers={"Authorization": f"Bearer {token}"}
    )
    list_resp.raise_for_status()
    list_id = list_resp.json()["id"]

    # Finally get attachments
    url = f"{GRAPH_API_ENDPOINT}/sites/{site_id}/lists/{list_id}/items/{item_id}/attachments"
    headers = {"Authorization": f"Bearer {token}"}
    ...
    try:
        ...
        print(f"✅ Found {len(attachments)} attachment(s) for item {item_id}")
        return attachments
    except requests.exceptions.RequestException as e:
        print(f"❌ Error fetching attachments for item {item_id}: {e}")
        return None
